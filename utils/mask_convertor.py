...
class DendriteSWC():
    ...
    def generate_tif(self, swc_file, filename):
        target = swc_file[:-4] + "_maskdendrite.tif"
        self.open(swc_file, filename)
        self.convert(target)
        test_dendrite = np.asarray(io.mimread(target, memtest=False))
        return target
