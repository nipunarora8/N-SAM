...
class MetricsStatistics:
    def __init__(self):
        ...
        self.save_dir = ''
        self.metric_values = defaultdict(list) # check epoch
        self.metric_epochs = defaultdict(list) # epoch

    def cal_epoch_metric(self, label_type, label, pred):
        for x in self.func_dct.keys():self.metric_values["{}-{}".format(x, label_type)].append(self.func_dct[x](label, pred))
    ...
    def cal_jaccard_index(self, pred, label, score=0.5):
        pred = pred > score
        label = label > score
    ...
    def cal_dice(self, pred, label, score=0.5):
        pred = pred > score
        label = label > score
        intersection = (pred & label).sum().item()
